...
def clean(
    ...
    chunk_size: int = 150,  # 80 → 150 (API 호출 횟수 감소)
) -> CleanedResult:
    ...
    total = len(segments)
    print(f"[Cleaner] Start: {total} segments (chunk_size={chunk_size})")
    ...
    for chunk_start in range(0, total, chunk_size):
        ...
        if progress_callback:
            pct = 66 + int((chunk_start / total) * 14)
            progress_callback(f"Gemini preprocessing... ({chunk_start}/{total})", pct)
        ...
        result_chunks = _call_gemini(client, raw_segs)
...
2. Join incomplete sentences with next segment
3. Group 2-5 related segments into one paragraph
4. s = first segment start, e = last segment end
5. Keep all content - do not skip or summarize
...
[{{"s": 0.0, "e": 5.2, "t": "cleaned text"}}, ...]"""
    ...
    try:
        ...
        print(f"[Cleaner] Parse failed, keeping original: {e}")
